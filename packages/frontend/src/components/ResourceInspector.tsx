...
export function ResourceInspector({resource, object}: ResourceInspectorProps) {
    ...
    const isFunction = resource.service === 'function' || resource.type === 'function'
    const isEvents = resource.service === 'events' || resource.type === 'topic'
    const isObservability = resource.service === 'observability' || resource.type === 'log-group'
    ...
    return (
        <aside className="resource-inspector">
            ...
            {isQueue && <QueueDetailsSection metadata={resource.metadata}/>}
            {isFunction && <FunctionDetailsSection metadata={resource.metadata}/>}
            {isEvents && <EventDetailsSection metadata={resource.metadata}/>}
            {isObservability && <ObservabilityDetailsSection metadata={resource.metadata}/>}
...
function FunctionDetailsSection({metadata}: {metadata: Record<string, unknown>}) {
    const runtime = getStringMetadata(metadata.runtime)
    const handler = getStringMetadata(metadata.handler)
    const memory = metadata.memorySizeMb !== null && metadata.memorySizeMb !== undefined ? `${metadata.memorySizeMb} MB` : null
    const timeout = metadata.timeoutSeconds !== null && metadata.timeoutSeconds !== undefined ? `${metadata.timeoutSeconds}s` : null
    const arn = getStringMetadata(metadata.arn)

    return (
        <section className="inspector-section">
            <p className="metric-label">Function Details</p>
            <div className="inspector-grid compact-grid">
                {runtime && <InspectorItem label="Runtime" value={runtime}/>}
                {handler && <InspectorItem label="Handler" value={handler}/>}
                {memory && <InspectorItem label="Memory" value={memory}/>}
                {timeout && <InspectorItem label="Timeout" value={timeout}/>}
            </div>
            {arn && (
                <div style={{marginTop: 8}}>
                    <p className="metric-label">ARN</p>
                    <code style={{fontSize: 11, wordBreak: 'break-all', display: 'block', marginTop: 4, color: 'var(--link)'}}>{arn}</code>
                </div>
            )}
        </section>
    )
...
function EventDetailsSection({metadata}: {metadata: Record<string, unknown>}) {
    const topicType = getStringMetadata(metadata.topicType)
    const arn = getStringMetadata(metadata.arn)

    return (
        <section className="inspector-section">
            <p className="metric-label">Topic Details</p>
            <div className="inspector-grid compact-grid">
                {topicType && <InspectorItem label="Type" value={topicType}/>}
            </div>
            {arn && (
                <div style={{marginTop: 8}}>
                    <p className="metric-label">Topic ARN</p>
                    <code style={{fontSize: 11, wordBreak: 'break-all', display: 'block', marginTop: 4, color: 'var(--link)'}}>{arn}</code>
                </div>
            )}
        </section>
...
function ObservabilityDetailsSection({metadata}: {metadata: Record<string, unknown>}) {
    const retentionDays = metadata.retentionDays !== null && metadata.retentionDays !== undefined
        ? (metadata.retentionDays === 0 ? 'Never expire' : `${metadata.retentionDays} days`)
        : 'Never expire'
    const storedBytes = typeof metadata.storedBytes === 'number'
        ? formatBytes(metadata.storedBytes)
        : null

    return (
        <section className="inspector-section">
            <p className="metric-label">Log Group Details</p>
            <div className="inspector-grid compact-grid">
                <InspectorItem label="Retention" value={retentionDays}/>
                {storedBytes && <InspectorItem label="Stored Data" value={storedBytes}/>}
            </div>
        </section>
    )
}

function QueueDetailsSection({metadata}: {metadata: Record<string, unknown>}) {
    const url = getStringMetadata(metadata.url)
    const queueType = getStringMetadata(metadata.queueType)
    const messageCount = metadata.messageCount !== null && metadata.messageCount !== undefined
        ? String(metadata.messageCount)
        : null

    return (
        <section className="inspector-section">
            <p className="metric-label">Queue Details</p>
            <div className="inspector-grid compact-grid">
                {queueType && <InspectorItem label="Type" value={queueType}/>}
                {messageCount !== null && <InspectorItem label="Messages (approx)" value={messageCount}/>}
            </div>
            {url && (
                <div style={{marginTop: 8}}>
                    <p className="metric-label">Queue URL</p>
                    <code style={{fontSize: 11, wordBreak: 'break-all', display: 'block', marginTop: 4, color: 'var(--link)'}}>{url}</code>
                </div>
            )}
        </section>
    )
