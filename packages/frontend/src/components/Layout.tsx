...
const CLOUD_SERVICE_ITEMS: Array<{name: CloudSidebarService; label: string; route?: string}> = [
    ...
    {name: 'function', label: 'Function', route: 'function'},
    {name: 'events', label: 'Events', route: 'events'},
    {name: 'observability', label: 'Observability', route: 'observability'},
]
        ...
        <div className="nav-section cloud-service-nav">
            ...
            {CLOUD_SERVICE_ITEMS.map((service) => {
                ...
                const available = service.name === 'storage'
                    || (['k8s', 'database', 'compute', 'networking', 'queue', 'function', 'events', 'observability'].includes(service.name) && cloud === 'aws')
